        ...
        def filterHonors(self, request):
            if request.method == "POST":
                unique_depts = HonorsModel.objects.values_list(
                    'dept', flat=True).distinct()
                try:
                    for dept in unique_depts:
                        top_records = HonorsModel.objects.filter(
                            dept=dept).order_by('-scgpa')[:5]
                        top_record_ids = top_records.values_list(
                            'rollno', flat=True)
                        HonorsModel.objects.filter(dept=dept).exclude(
                            rollno__in=top_record_ids).delete()

                    for index, record in enumerate(top_records, start=1):
                        record.selectedDept = f'{dept}_TOP{index}'
                        record.save()
                    print(len(top_record_ids))
